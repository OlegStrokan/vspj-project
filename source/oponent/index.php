...
$sql = "SELECT id, nazev, cislo, stav.popis, posudek.datum FROM posudek INNER JOIN prispevky ON posudek.id_prispevek = prispevky.id  INNER JOIN stav ON prispevky.stav = stav.id_stav WHERE stav = 3 AND id_oponent = (SELECT userid FROM users WHERE username = '".$_SESSION['login_user']."' )";
$result = mysqli_query($connection, $sql);

$sql_pridelene = "SELECT id, nazev, cislo, stav.popis, posudek.datum FROM posudek INNER JOIN prispevky ON posudek.id_prispevek = prispevky.id  INNER JOIN stav ON prispevky.stav = stav.id_stav WHERE (stav = 5 OR stav = 6) AND id_oponent = (SELECT userid FROM users WHERE username = '".$_SESSION['login_user']."' )";
$result_pridelene = mysqli_query($connection, $sql_pridelene);

$sql_odm = "SELECT id, nazev, cislo, stav.popis, posudek.datum FROM posudek INNER JOIN prispevky ON posudek.id_prispevek = prispevky.id  INNER JOIN stav ON prispevky.stav = stav.id_stav WHERE stav = 4 AND id_oponent = (SELECT userid FROM users WHERE username = '".$_SESSION['login_user']."' )";
